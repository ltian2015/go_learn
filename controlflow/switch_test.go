...
先对switch 匹配目标表达式sw_exp求值，得到一个结果，也就是匹配目标值。
然后再按照顺序，对case分支的条件表达式进行求值，也就是分支条件值，如果分支条件值与匹配目标值
相等（也就是匹配），就执行该case分支下的代码，如果在该分支最后没有fallthrough命令，
就自动break，结束整个switch（这与C++的switch不同，c++必须写break才不会进入下一个分支代码）。如果一直没有遇到匹配的case分支，但存在default分支，就执行该default分支。
如果相匹配分支的最后一条分支代码语句是fallthrough命令，那么不会自动结束switch，而是忽略下个分支的表达式的求值与匹配，继续“直接、执行”下一个分支代码。
case分支代码中的fallthrough命令主要用于上个分支条件符合或满足下个分支条件，而下个分支条件不满足上个分支条件的逻辑情况。这样，当上一个
