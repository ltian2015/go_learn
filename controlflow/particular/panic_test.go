...
2.3 panic(nil)也会导致goroutine处于panic状态，不使用recover()函数移除panic状态，goroutine仍会崩溃，
    要注意的是，在1.21版之前，recover()函数所捕获的panic值是nil，从1.21版开始之后，recover()函数所捕获的panic值
	将不在是nil，而是*runtime.PanicNilError.
...
2.5 想要使用recover()捕获得到函数f（或其嵌套调用的函数）中发生的panic，
   ...
   recover()才能生效。即：
  rule1. 在函数f的函数体中直接使用defer recover()函数无法消除函数f中的panic。
...
func TestPanicInOneGoroutine(t *testing.T) {
	...
	println("unreachable ") //注意，这一行执行不到了。因为前面发生了panic
}

// 展示单协程中抛出panic，并用defer+recover恢复异常的情景。
func TestPanicRecoverInOneGoroutine(t *testing.T) {
	defer func() { //这是在主函数退出阶段执行的第二个函数
		println("programe exit here") //注意，尽管退出阶段执行的第一个函数中抛出了panic,但不影响，仍会会被打印。
	}() //将在由于panic所导致的退出阶段时执行该函数。
	defer func() { //这是在主函数退出阶段执行的第一个函数
		v := recover()
		fmt.Println("recovered:", v) //注意，此处捕获上级函数正常运行阶段的抛出的panic
		panic("bye again")           //注意，在退出阶段执行的第一个函数中再次抛出Panic，但不影响退出阶段后续函数的执行
...
// 与上个函数区别在于，在函数退出阶段前期所执行的函数所抛出的异常被后期执行的函数所捕获（处理并清除）
func TestPanicRecoverInOneGoroutine2(t *testing.T) {
	defer func() { //这是在主函数退出阶段执行的第二个函数
		v := recover()
...
/**
   想要使用recover()捕获得到函数f（或其嵌套调用的函数）在正常阶段中发生的panic，
   recover()必须在正确的位置调用，也就是在f函数的某一个defer函数（假定为deferedFn）的代码中直接调用
   recover()才能生效。即：
