...
滥用，GO不允许内存布局兼容的两类型之间进行隐式（直接）的类型转换，必须使用显式（强制）的类型转换，从而提高了类型安全。
 ...
 1. 值的类型转换语法。
    ...
    注意，“简化类型转换写法”常用于将值转换为“已定义类型”，而 “正规类型转换写法”往往用于将值转换为“未经定义的类型”，
    这是因为，未经定义类型是多个类型组成的复合类型，也就是字面类型，因此被()括起来的阅读效果更好。
	...
	注意：类型转换方式(T)(v)之所以是在编译时可以完成的静态类型转换，而ValueT,boolValue= VI.(T)之所以
    !!! 是只能在程序运行时动态完成的类型转换，是因为第一种方式下，值v的类型在编译时是确定的。
    !!! 而第二种方式下，接口值VI的类型在编译期间无法确定，在运行时才能确定具体类型，因此，只能在运行时进行类型转换。
*****************************************************************************
*/
//------golang 字符串的转换--------------------------
//golang中字符串采用utf-8编码，utf-8编码是unicode字符集的一种变长编码规则，
//对于不同国家的字符，所占用的字节数不同，比如，英文字母只占一个字节，而中文则占3个字节。
//!!! 所以，golang中，字符串的字节数与字符数不相等。
//!!! 也因此，对于字符串s，如果i是一个大于0，小于字符串字节数的整数，那么，s[i]得到只是一个byte，可能不是一个完整字符。
//!!! 想要得到字符串中的字符数，可以显式地将字符串s转换为rune切片，比如： chars:=[]rune(s)
//!!! 然后再从rune切片中得到指定下标位置的字符。

func TestStringCovert(t *testing.T) {
	var s = "Hello 中国 is a string"
	fmt.Printf("字符串：\"%s\" 的字节数是%v\n", s, len(s))
	//将字符串显式地转换为rune切片
	var chars []rune = []rune(s)
	fmt.Printf("字符串：\"%s\" 的字符数是%v\n", s, len(chars))
	var byteAt6 byte = s[6]
	fmt.Printf("字符串：\"%s\" 第6个字节是：%c\n", s, byteAt6)
	var charAt6 rune = chars[6]
	fmt.Printf("字符串：\"%s\" 第6个字符是：%c\n", s, charAt6)
}
...
2.值的类型隐式（直接）转换规则 ：
	...
	但是为了避免内存布局兼容的变量之间的发生无意地类型转换（隐式转换）而造成的误操作，
	!!! GO制定了严格地类型转换规则，只有当同一类型具有不同类型名称是，才允许隐式的转换。
...
2.2 与底层类型相关的转换规则。对与Tx类型的值x和"非接口类型"T，
	...
	（2）如果二者（Tx 和T）之中有一个是未经定义的类型（字面类型），并且二者的底层类型相同（要考虑struct中的元数据
	    tag，，那么x就可以隐式地转换为类型T。
		!!! 规则解析：因为未经定义的类型（字面类型）没有方法集（方法集为空，空集既可以理解为包含所有方法，所以作为
		源类型，可以转换成有限方法集的目标类型；又可以理解为什么方法都没有，所以作为目标类型，无方法集可用，比较安全），
		!!! 总之，底层类型相同的两个可以互转换的类型中，只要两个类型值之中有一个是字面类型，那么二者之间不存在方法集的差异，可以隐式转换。
		!!!! 类型转换一定是类型操作安全的，但是考虑到struct的tag元数据定义在类型上，通过类型的反射操作对元数据处理不同，
	...
	（3）如果二者（Tx和T）的底层类型不同，但是 ，如果Tx和T都是未定义的指针类型（指针字面类型），并且，指向的
		...
		进行操作。
		!!! s规则解析：对于两个未定义的指针类型（指针字面类型）Tx和T，指针类型所指向的底层数据内存布局一样，
		这样为相同内存布局换另一套方法集（指针字面类型的方法集取决于于所指向类型的方法定义），就不会造成内存错误，
		只不过是为了避免不经意的逻辑错误，需要程序员作显式（明显的）类型转换。
