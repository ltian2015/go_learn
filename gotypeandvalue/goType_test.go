...
在GO1.9之前就是通过是否有名字来对类型进行分类，但是随着可以为类型定义“别名（alias）”之后，按照是否有名字给
...
如果“类型别名（type alias）”实际上是“字面类型”，也就是未定义类型，那么尽管看起来有个类型名字，但是不能向已定义类型那样
为其定义“方法集”。
...
注意：对于编译器而言，已定义类型（命名类型）会被存储在程序文件中的某个位置。未经定义的类型（字面）则按照出现的位置存储。
注意：未经定义的类型的别名在编译的时候会被替换为字面类型，存储在出现的位置。
...
func TestUseUnnamedTypeVar(t *testing.T) {
...
/***********************************************************************************************
  ...
  方法集范围判定规则：
   ...
   注意：因此，当我们写下type A B 语句来定义新类型A时，类型A没有类型B的任何方法。
...
func TestShowTypeMehodSet(tc *testing.T) {
...
/*********************************************************************************************
  四、何为类型的底层类型？知道类型的底层类型有什么用途？
   ...
   注意：在GO中，一个类型的底层类型（Underlying Type）决定了该类型的值在内存中的实际结构。
   ...
   也就是“组合类型”（比如[]T,Map[A]B），就终止遍历，此时得到的类型就是该类型的底层类型。
   注意，遇到的“组合类型”不一定是预先定义的基本类型的组合，遇到的是衍生类型的组合类型也要终止遍历 （因为，组合类型可以根据被组合的类型的底层类型来确定存储结构）。
   注意，因此，类型的底层类型都是预先定义的boolean、 numeric、string类型或“字面类型（组合类型）”。
...
type (
	...
	MyIntSlice []MyIntType // 底层类型是 []MyIntType ，遇到的引用类型是组合类型而截止。
