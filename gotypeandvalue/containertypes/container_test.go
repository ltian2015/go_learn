...
// !!! GO语言中，类型的一个作用是决定”值“的内存布局。不同类型的值的内存布局不同，有的类型只有单独一段内存存储该类型的数据值，被称为”直接值部Direct Value Part“
// !!! 有的类型不仅有直接值部，还有”底层值部（Underlying Value Part）“，真实的数据往往存储在”底层值部“之中。
// !!! 在基本类型中，除了string类型有直接值部和底层值部外，其他基本类型，都只有一个直接值部，没有“底层值部（Underlying Value Part）”
// !!! 比如,数组类型数据就只有一个连续的内存段作为“直接值部（Direct Value Part）”，数组中所有元素的数据都存储在“直接值部”内，数组的拷贝会拷贝源数组的内容。
// !!! 在非基础类型的复合类型中，除了struct类型只有”直接值部“，没有”底层值部“外，其他的类型不仅有直接值部，而且也有底层值部。
// 注意： 一、 所有通过变量赋值形式进行的值拷贝，都是拷贝值的“直接值部（Direct Value Part）”，而不拷贝“底层值部”。
// 注意： 二、SizeOf函数也是用于度量“直接值部（Direct Value Part）”的字节大小，对于Slice、Map、Channel、function、Interface、String
//       !!! 这些有“底层值部”的类型的变量，使用SizeOf测定其内存大小，得到的都是相同的值，也就是直接值部的固定字节数。
//       !!! 不能反映真实内存的占用。
// !!! 而SizeOf函数也是用于度量“直接值部（Direct Value Part）”的字节大小。故而，凡是内容存储在"底层值部"中的
// !!! 容器类型的值，其SizeOf函数测量的内存占用都是一样的，都是“直接值部”所占用的内存大小，
...
// !!!（1）对于map[K,V]，K必须是值可比较的类型。
// !!! (2)map的拷贝同slice拷贝一样，拷贝了直接值部，两个map变量通过直接值部共享一个底层值部，这样，任何一个
...
// !!! (3)使用K的值k对map进行取值时，当m为nil，或者m[k]的值不存在时，都不会抛出panic，
// !!!    而会返回对应类型的零值（注意，有底层值部和无底层值部的零值不同，有底层值部的类型的零值是nil）.
// !!! (4)同时,m[k]会返回两个值，第二个值的类型为bool,表示元素是否存在,第二个返回值可以忽略，故而
