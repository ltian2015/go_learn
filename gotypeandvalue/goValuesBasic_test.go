...
那么值也分为有名字的值与无名字的值。为值设定名字（标识符）是为了程序员在程序中反复多次引用，有名字的值又分为“常量”和“变量”，
...
通道（channel）、接口（interface）无法写出字面量。
!!! 因为指针类型的变量存储的数据是运行时的内存地址，程序员在编写程序时无法给出。
!!! 而通道（channel）则是一个数据通信的协调器，不是一个数据，无需给出字面量。
...
// var pi *int64 = &(p - int64(0)) //!!!表达式也不能取地址
...
/********************************************************************************************
3.!!!unsafe包中函数求值的常量特征。
   !!!unsafe包中的函数都是在编译期间求值，因此，可以看作是常量表达式的组成部分。
  !!! 所以，只要unsfe包中的函数有返回值，且返回值是可以用来定义常量的基本类型，
  !!! 那么就可以用这个函数调用表达式来定义常量。
**********************************************************************************************/
...
4、iota【a:juta】在常量声明中的用途。
 ...
 !!! 在golang中，iota主要用于简化“批量式”的常量定义，
 !!! iota出现在一个有效常量定义行中的时候，它表示计数0，每出现一次就自动加1
 !!! 如果后面的常量定义行（非注释和空行）不写任何代码，编译器会自动复制上一行中含iota的常量表达式,来定义该常量，
 !!! 如果后面的常量定义行中写了含iota的常量表达式，则编译器会自动取iota的当前值。
 !!! 当然，如果常量不在同一批中，则iota互不相让影响。
**********************************************************************************************/
//--------------------------------- iota 在定义常量中的用途示例 开始---------------------------//
